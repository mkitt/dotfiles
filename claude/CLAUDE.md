...
- Ephemeral drafts live in the session scratchpad directory, never in the repo — nothing lands in the working tree unless I ask for it
- For substantive work that has both a PR and a Linear issue, publish that working doc as a **single HTML artifact and link it from both** — one shared visual source of truth (design, diagrams, decisions, evidence), not separate inline diagrams per surface. Same file path on republish keeps the URL stable. Skip for flat fixes with no structure worth a doc.
- **Sharing gate:** artifacts are default-private — before linking, remind me to share it with the right audience (reviewers, teammates); ask who should see it if unclear.
